import ReminderProps from "@/types/reminder";
...
import { fetchAdditional } from "../actions";
...
export default async function Type({
  ...
  async function fetchType() {
    const res = await fetch(
      `${process.env.BACKEND_URL}/reminder?s=${search}&sort=${sort}&order=${order}&limit=${limit}&page=${page}`,
      {
        cache: "force-cache",
        next: {
          tags: ["reminder"],
  ...
  const reminders: ReminderProps[] = typeData.data;
  ...
  //@ts-ignore
  const data: ReminderProps[] = await Promise.all(
    reminders.map(async (remind, index) => ({
      ...remind,
      id: String(index + start),
      typed: remind.type ? (
        <div className="flex flex-col">
          {remind.type.map(async (tp, i) => {
            const a = await fetchAdditional("type", tp, "name");
            return <p key={i}>{a}</p>;
          })}
        </div>
      ) : null,
      createdAt: convertISOToReadable(remind.createdAt),
      updatedAt: convertISOToReadable(remind.updatedAt),
    }))
  );
  ...
  return (
    <>
      <header className="h-12 pt-20 md:pt-0 flex items-center justify-between w-full px-3 md:px-5 relative">
        <Filter headers={filters} placeholder="Reminder" />
      </header>
